...
找到自己合适大小的位置并`插`入 + 让更大的全部后`挪`。时间复杂度是 **O(n2)**

就像玩扑克牌那样，第一张就放着做基础牌；从第`二`张牌开始，与`前边`集（先只有第一张）对比，更大的统统往后移动一位，没有（更大的）就直接插入当前牌；最后一张，也一样完成。

  - 第一张牌直接不动：（Base Case）

    就待在原处。

  - 从`第二`张牌开始，当做`新抽到的新牌`，开始做循环处理 ：（递推关系）

    对比第一张牌，看看要不要让（第一张牌）它往后挪；

      - 如果需要它往后挪，就将`新牌`插入前边第一张牌的位置；

      - 如果不需要它往后挪，就直接插入当前（第二张）位置；
...
找到全局最小的，与`第一个`位置的交换（位置）；

再找到全局最小的（排除第一个），与`第二个`位置的交换（位置）
...
一个一个的增量查（一个个对比）。时间复杂度为 **O(n)**，线性的。

- 还有没有更快的算法？比如时间复杂度为 O(lgn) 的？

- 时间复杂度和`排序算法`(**O(n2)**)相同的`查找算法`为：

  先把这些数**排序**（就像数据库的索引），然后**取**第k个，。
...
前提：如果是 从一组`排好序的`序列里找出某个元素的位置，则可以有更快的算法。时间复杂度为？O(n)，？线性的。

- 有了索引之排序，就好说了，直接二叉树查找
  ...
  int binarysearch(int number)
  {
          ...
          while (start <= end)
          {
            ...
            // 判断目标值和 mid 的区域在二分的哪边。并据此重定义范围以逼近目标值(例如现在先把搜索范围缩小到后半部分)。。。直到 Base Case：return mid;
            if (a[mid] < number)
              start = mid + 1;
